...
public class FixTest {
	...
	SimpleOrderRepo parentOrderRepo = new SimpleOrderRepo<>(new ArrayDeque<>(), ()->new Parental(idgen), new HashSet<>(), new HashMap<>(), new HashMap<>(), new HashMap<>());
	SimpleOrderRepo childOrderRepo = new SimpleOrderRepo<>(new ArrayDeque<>(), ()->new Order(idgen), null, new HashMap<>(), new HashMap<>(), new HashMap<>());
	SimplePool simplePool = new SimplePool<>(new ArrayDeque<>(), ()->new Order(idgen), 10);
	UpstreamPublisher toUpstream = new UpstreamPublisher();
	UpstreamHandler fromUpstream = new UpstreamHandler(idgen, toUpstream, parentOrderRepo, simplePool, validators);
	DownstreamHandler fromDownstream = new DownstreamHandler(idgen, childOrderRepo);
