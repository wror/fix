...
import broke.fix.misc.ExecutionRepo;
...
public class DownstreamHandler<F extends FixFields> {
	...
	private ExecutionRepo execRepo;
	...
	@Inject
	public void setExecutionRepo(ExecutionRepo execRepo) {
	...
	public void handleExecutionReport(ExecType execType, long transactTime, CharSequence downstreamOrderID,
		...
		switch (execType) {
			case Rejected:
				...
				break;
			case New:
				accept(order, downstreamOrderID);
				break;
			case Trade:
			...
			case Replaced:
				...
				repo.updateClOrdID(origClOrdID, clOrdID, order);
	...
	private void accept(Order<F> order, CharSequence downstreamOrderID) {
