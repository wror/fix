...
public class UpstreamHandler<F> {
	...
	@Inject
	public UpstreamHandler(IdGenerator idgen, OrderListener<F, Parental<F>> publisher, SimpleOrderRepo<F, Parental<F>> repo, SimplePool<Order<F>> pool, Collection<Validator<F>> validators) {
		this.idgen = idgen;
	...
	public void handleReplace(CharSequence orderID, CharSequence clOrdID, long origOrdModTime, long orderQty, F fields) {
		if (isDupe(clOrdID)) {
